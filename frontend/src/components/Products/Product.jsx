...
const Product = ({ _id, name, images, ratings, numOfReviews, price, cuttedPrice }) => {
    ...
    return (
        <div className="group bg-white rounded-2xl border border-slate-100 hover:border-amber-300 shadow-xs hover:shadow-lg transition-all duration-200 p-3.5 flex flex-col justify-between relative overflow-hidden">
            ...
            <Link to={`/product/${_id}`} className="flex flex-col items-center mt-2">
                <div className="w-40 h-40 sm:w-44 sm:h-44 rounded-xl overflow-hidden bg-slate-50 flex items-center justify-center p-2 group-hover:scale-105 transition-transform duration-200">
                    <img draggable="false" className="w-full h-full object-contain" src={images && images[0] ? images[0].url : ""} alt={name} />
                </div>

                <h3 className="text-xs sm:text-sm font-semibold text-slate-800 group-hover:text-amber-600 transition mt-3 text-center line-clamp-2 h-10 px-1">
                    {name}
                </h3>
            </Link>
